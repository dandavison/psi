...
on division-by-loci. One reason for doing so is the number of loci is
typically larger than the number of individuals and therefore more
often exceeds \p, the number of processes.
#+begin_latex
  \begin{algorithm}
