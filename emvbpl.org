...
#+begin_latex
\begin{algorithm}
  \caption{EM algorithm: No Admixture model}
  \label{alg:em-nam}
  \begin{lstlisting}[mathescape,escapeinside='']
    '\textbf{E step}'
    ...
    '\foreach individual $i$'
        ...
        '\foreach group $k$'
            $\gamma_i^k \gets \gamma_i^k / \sum_{k'} \gamma_i^{k'}$
...
#+begin_latex
\begin{algorithm}
  \caption{VB algorithm: No Admixture model (Overview)}
  \label{alg:vb-nam}
  \begin{lstlisting}[mathescape,escapeinside='']
    ...
    '\foreach individual $i$'
        '\foreach group $k$'
            $\gamma^{i}_{k} = \exp\left\{\E_{q(\theta)} \log p(z_{i}=k,x_{i}|\theta)\right\}$
        '\foreach group $k$'
            $\gamma^i_k \gets \gamma^i_k / \sum_{k'} \gamma^i_{k'}$
...
#+begin_latex
\begin{algorithm}
  \caption{VB algorithm: No Admixture model (Explicit)}
  \label{alg:vb-nam}
  \begin{lstlisting}[mathescape,escapeinside='']
    ...
    '\foreach individual $i$'
        '\foreach locus $l$'
            '\foreach chromosome $a$'
                ...
                '\foreach group $k$'
                    $\gamma_{ilak} \gets \gamma_{ilak} / \sum_{k'} \gamma_{ilak'}$
